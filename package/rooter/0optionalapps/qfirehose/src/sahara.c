...
static int start_image_transfer(void *usb_handle, void *tx_buffer, const struct sahara_pkt *sahara_read_data, FILE *file_handle)
{
    int retval = 0;
    uint32_t bytes_read = 0, bytes_to_read_next;
    uint32_t DataOffset = le_uint32(sahara_read_data->q_sahara_read_packet_data.q_data_offset);
    uint32_t DataLength = le_uint32(sahara_read_data->q_sahara_read_packet_data.q_data_length);

    dbg(LOG_INFO, "0x%08x 0x%08x 0x%08x", le_uint32(sahara_read_data->q_sahara_read_packet_data.q_image_id), DataOffset, DataLength);
...
static int start_image_transfer_64bit(void *usb_handle, void *tx_buffer, const struct sahara_pkt *sahara_read_data_64bit, FILE *file_handle)
{
    struct sahara_pkt sahara_read_data;

    sahara_read_data.q_header = sahara_read_data_64bit->q_header;
    sahara_read_data.q_sahara_read_packet_data.q_image_id = sahara_read_data_64bit->q_sahara_read_packet_data.q_image_id;
    sahara_read_data.q_sahara_read_packet_data.q_data_offset = sahara_read_data_64bit->q_sahara_read_packet_data.q_data_offset;
    sahara_read_data.q_sahara_read_packet_data.q_data_length = sahara_read_data_64bit->q_sahara_read_packet_data.q_data_length;
...
static int sahara_flash_all(void *usb_handle, void *tx_buffer, void *rx_buffer, FILE *file_handle) {
    ...
    while (1)
    {
        ...
        {
            start_image_transfer_64bit(usb_handle, tx_buffer, pr_sahara_pkt, file_handle);
        }
...
int sahara_main(const char *firehose_dir, const char *firehose_mbn, void *usb_handle, int edl_mode_05c69008) {
    ...
    if (edl_mode_05c69008)
    {
        ...
        if (is_upgrade_fimeware_zip_7z)
        {
            ...
            {
                if ((strstr(file_name_b.file_backup_c[i].zip_file_name_backup, "NPRG9x") && strstr(file_name_b.file_backup_c[i].zip_file_name_backup, ".mbn")))
                {
                    ...
                    {
                        memmove(prog_nand_firehose_filename_tmp, file_name_b.file_backup_c[i].zip_file_name_backup, strlen(file_name_b.file_backup_c[i].zip_file_name_backup));
                        memmove(prog_nand_firehose_filename_dir_tmp, file_name_b.file_backup_c[i].zip_file_dir_backup, strlen(file_name_b.file_backup_c[i].zip_file_dir_backup));
                        break;
                    }
                }
            }
            ...
            if (prog_nand_firehose_filename_tmp[0] != '\0')
            {
                ...
                {
                    snprintf(zip_cmd_buf, sizeof(zip_cmd_buf), "unzip -o -q %.240s '*%.200s' -d /tmp/ > %s", firehose_dir, prog_nand_firehose_filename_dir_tmp, ZIP_PROCESS_INFO);
                }
                else
                {
                    snprintf(zip_cmd_buf, sizeof(zip_cmd_buf), "7z x %.240s -o/tmp/ %.200s > %s", firehose_dir, prog_nand_firehose_filename_dir_tmp, ZIP_PROCESS_INFO);
                }
